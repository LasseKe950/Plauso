...
//               printf("Event = %d\n", e.type);
...
#endif
               {
                  ...
                  if(e.type == SDL_MOUSEMOTION)
                  {
                     x = e.motion.x; 
                     y = e.motion.y;
#ifdef TESTIPADAN_NAYTTO
                     //uusi näyttö 8.11
                     int x0, y0;
                     x0 = x;
                     y0 = y;
                     x = 800 - (float)y0*800/1280;
                     y = (float)x0 * (float)1280/800;
                     //uusi näyttö 8.11
#endif
                  }
                  else
...
//printf("MOUSEBTN DOwN x=%d y=%d\n", x, y);
                  ...
                  if( e.type == SDL_FINGERDOWN || e.type == SDL_FINGERUP)
                  {
                     ...
                     y0 = e.tfinger.y * gScreenRect.w;
                     x0 = e.tfinger.x * gScreenRect.h;
   ...
   printf("x0 = %d y0 = %d\n", x0, y0);
   printf("Fx = %d Fy = %d --%c\n", x, y, e.type == SDL_FINGERDOWN ? 'D' : 'U');
//   printf("Mx = %d My = %d\n", (4096/x) * 800, (4096/y)*1280);//#Datamodule touch -> -DPC_UBUNTU...? 4096x4096 ??
                  }
#endif
                  else
                     x = y = -1;
            ...
            //if (debug_ok && (user_profile.user_level == 2)) //Debugataan korttien tilatietoja
            //   debug_show_tilatiedot();
